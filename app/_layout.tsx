...
function RootLayoutNav() {
  return (
    <Stack screenOptions={{ headerShown: false, animation: "slide_from_right" }}>
      ...
      <Stack.Screen name="voting-result" options={{ gestureEnabled: false }} />
      <Stack.Screen name="rules" />
      <Stack.Screen name="about" />
      <Stack.Screen name="custom-slot/[slot]" />
    </Stack>
  );
}
...
export default function RootLayout() {
  ...
  // Initialize Yandex Mobile Ads SDK on native platforms
  useEffect(() => {
    if (Platform.OS === "web") return;
    try {
      const { MobileAds } = require("yandex-mobile-ads") as {
        MobileAds: { initialize: () => void };
      };
      MobileAds.initialize();
    } catch (e) {
      console.warn("[YandexAds] SDK not available (Expo Go?):", e);
    }
  }, []);
